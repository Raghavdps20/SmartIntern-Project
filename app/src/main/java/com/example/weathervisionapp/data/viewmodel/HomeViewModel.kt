...
import com.example.weathervisionapp.data.local.Constants.DEFAULT_CITY
import com.example.weathervisionapp.data.local.Constants.IMPERIAL
...
import com.example.weathervisionapp.data.repository.HomeRepo
...
class HomeViewModel @Inject constructor(
    ...
    var tempUnit = ""
    val requestState: MutableState<RequestState> = mutableStateOf(
        RequestState.IDLE)
    private val cityState: MutableState<String> = mutableStateOf("")

    private val _geocoding = MutableStateFlow<Resource<List<GeocodingResponse>>>(
        Resource.Idle)
    val geocoding: StateFlow<Resource<List<GeocodingResponse>>> = _geocoding

    private val _currentWeather = MutableStateFlow<Resource<CurrentWeatherResponse>>(
        Resource.Idle)
